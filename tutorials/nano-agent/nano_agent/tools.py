...
def get_tool_schemas(include_subagents: bool = True) -> list[dict]:
    return [
        schema for name, (schema, _) in TOOL_REGISTRY.items()
...
@tool("bash", "Run a shell command and return its output.", {
...
@tool("write_file", "Write content to a file. Creates parent dirs if needed.", {
    "file_path": {"type": "string", "description": "Absolute or relative path to the file"},
...
@tool("grep", "Search for a regex pattern in files. Returns matching lines with paths and line numbers.", {
...
@tool("list_directory", "List files and directories at the given path. Defaults to current directory.", {
...
def register_subagent_tool(run_fn):
    @tool("run_subagents", "Run independent tasks in parallel using sub-agents. Use when tasks don't depend on each other.", {
        "tasks": {
            "type": "array",
            "items": {
                "type": "object",
                "properties": {"task": {"type": "string", "description": "Task for the sub-agent"}},
                "required": ["task"],
            },
